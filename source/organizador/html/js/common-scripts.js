...
$(function(){
    ...
    function getDataRegister(){
        database.on('value', function(api){
            ...
            var dataFireBase = [];
            var rowsData = null;
            console.log('se agrego uno mas');
            for(var key in dataRegister){
                dataFireBase = [dataRegister[key].NombresCompletos, dataRegister[key].ApellidosCompletos , dataRegister[key].Correo , dataRegister[key].DNI , dataRegister[key].Direccion, dataRegister[key].Telefonos, dataRegister[key].RUC, '<button class="btn btn-danger delete" data-eraser="'+ key +'"><span class="glyphicon glyphicon-trash"></span></button>'];
                rowShow.push(dataFireBase);
            };
            rowsData = $('#dynamic-table').DataTable( {
                data: rowShow,
                destroy: true,
                lengthMenu: [ 200, 5000],
                columns: [
            ...
            var allData = $('.js-data_table').data('dataTable', rowsData)
            if(dataFireBase != ''){
                $('.btn.delete').on('click', function(){
                    var elem = $(this).data('eraser');
                    var elementDelete = $(this).closest("tr");
                    var deleteItem = database.child(elem);
                    $('#dynamic-table').DataTable().rows(elementDelete).remove().draw();
                    deleteItem.remove();
                });
            }
        });
