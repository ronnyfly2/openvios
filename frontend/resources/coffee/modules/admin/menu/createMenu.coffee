yOSON.AppCore.addModule "createMenu", (Sb) ->
	type_menu = []
	arr_controllers = []
	optionsPlus =
		insertZonePlus: true
		isAllowed: (cEl, hint, target)->
	...
	suscribeEvents = ()->
		...
		$(document).on 'click', defaults.btn_delete_item, functions.deleteItemMenu
	functions =
		deleteItemMenu: ()->
			$this = $(this)
			alertify.confirm 'Esta seguro que desea eliminar el item.',
				()->
		...
		addItemMenu: ()->
			...
			if($(defaults.form_add_menu).parsley().isValid())
				$.fancybox.close(true)
				alertify.confirm '¿Está seguro que desea agregar un elemento al menu?',
					()->
						data_obj = {}
						data_obj.class_id = if $(defaults.classes_type).val() == "" then null else $(defaults.classes_type).val()
			...
			if item_new_menu.role_id && item_new_menu.menus
				$.ajax
					...
					complete: ()->
						...
						return
				.fail((xhr)->
					alertify.alert xhr.responseJSON.msg
				)
			else
